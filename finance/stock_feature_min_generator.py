...
class StockFeatureMinGenerator:
    ...
    def get_stock_min_data(self):
        ...
        if df is None or df.empty:
            raise ValueError(f"akshare未返回数据，可能是股票{self.stock_code}无分钟线或网络问题")
        # 自动检测时间列名
        time_col = None
        for col in ['datetime', '时间', 'date']:
            if col in df.columns:
                time_col = col
                break
        if time_col is None:
            raise ValueError(f\"未找到分钟线的时间列，实际返回列为: {list(df.columns)}\")
        df = df.rename(columns={time_col: 'datetime', '开盘': 'open', '收盘': 'close', '最高': 'high', '最低': 'low',
